...
export const EventsPaginatedList = ({ events }: { events: any }) => {
  return events ? (
    <>
      {events.pages.map((page: { offset: React.Key | null | undefined; results: any[] }) => (
        <Fragment key={page.offset}>
          {page?.results.map(
            (event: TransactionEvent, index: React.Key | null | undefined, arr: string | any[]) => (
